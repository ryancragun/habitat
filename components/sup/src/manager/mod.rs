...
impl Manager {
    ...
    fn new(cfg: ManagerConfig, fs_cfg: FsCfg, launcher: LauncherCli) -> Result<Manager> {
        ...
        let server = ButterflyServer::new(
            sys.gossip_listen(),
            sys.gossip_listen(),
            member,
            Trace::default(),
    ...
    fn add_service(&mut self, spec: ServiceSpec) {
        ...
        if feat::is_enabled(feat::InstallHook) {
            ...
            {
                if let Err(err) = command::package::install::check_install_hooks(
                    &mut UI::with_sinks(),
                    &package,
        ...
        events::event(events::ServiceStarted {
            ...
            .filter_map(|op| {
                match op {
                    ...
                    | ServiceOperation::Restart { to_stop: spec, .. } => {
                        ...
                        // restart future.
                        let f = self
                            .remove_service_from_state(&spec)
                            .map(|service| self.stop(service));
                        if f.is_none() {
                            // We really don't expect this to happen....
                            outputln!(
                                "Tried to remove service for {} but could not find it running, \
                                 skipping",
                                &spec.ident
                            );
                        }
